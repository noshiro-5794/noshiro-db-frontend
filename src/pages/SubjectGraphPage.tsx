...
const graphSectionLabelKeys: Record<(typeof graphSectionKeys)[number], 'graph.section.episodes' | 'graph.section.characters' | 'graph.section.staff' | 'graph.section.relations' | 'graph.section.meta'> = {
...
const graphNodeLabelKeys: Record<GraphNodeType, 'graph.subjectNode' | 'graph.staffNode' | 'graph.characterNode' | 'graph.episodeNode' | 'graph.relationNode' | 'graph.metaNode'> = {
...
function imageOf(item: Pick<SubjectSummary, 'image' | 'image_original' | 'image_thumbnail' | 'images'> | SubjectStaff | SubjectCharacter): string | null {
  const images = typeof item.images === 'object' && item.images ? item.images as { poster?: string | null; thumbnail?: string | null } : null;
  return stringOrNull(images?.poster) || stringOrNull(images?.thumbnail) || stringOrNull(item.image) || stringOrNull(item.image_thumbnail) || stringOrNull(item.image_original);
}
...
async function loadPages<T>(fetchPage: (query: PageQuery) => Promise<ApiPage<T>>, pageSize = 120, maxPages = Number.POSITIVE_INFINITY) {
  const results: T[] = [];
  let page = 1;
  ...
  do {
    ...
    page += 1;
  } while (next && results.length < count && page <= maxPages);
...
async function loadGraphPayload(subjectId: UUID, mode: GraphMode, sections: GraphSections): Promise<GraphPayload> {
  ...
  const [episodes, staff, characters, relations] = await Promise.all([
    sections.episodes ? loadPages((query) => indexApi.listSubjectEpisodes(subjectId, query), pageSize, maxPages) : Promise.resolve([]),
    sections.staff ? loadPages((query) => indexApi.listSubjectStaff(subjectId, query), pageSize, maxPages) : Promise.resolve([]),
    sections.characters ? loadPages((query) => indexApi.listSubjectCharacters(subjectId, query), pageSize, maxPages) : Promise.resolve([]),
    sections.relations ? loadPages((query) => indexApi.listSubjectRelations(subjectId, query), pageSize, maxPages) : Promise.resolve([]),
  ]);
...
function buildGraph({
  ...
  for (const episode of [...primaryEpisodes, ...otherEpisodes]) {
    ...
    addUniqueNode(nodes, {
      ...
      details: compactDetails([episode.type, episode.date, episode.ep_num ? `EP ${episode.ep_num}` : null, episode.sort ? `Sort ${episode.sort}` : null, episode.duration]),
      type: 'episode',
  ...
  for (const character of characters) {
    ...
    addUniqueNode(nodes, {
      ...
      details: compactDetails([character.role, character.type, character.gender, character.actors?.slice(0, 3).map((actor) => actor.name_cn || actor.name)]),
      type: 'character',
      size: isMain ? 24 : 18,
  ...
  for (const relation of relations) {
    ...
    addUniqueNode(nodes, {
      ...
      details: compactDetails([relation.relation, related.subject_type, related.platform, related.date, related.content?.episodes ? `${related.content.episodes} episodes` : null]),
      type: 'relation',
...
function drawGraph({
  ...
  for (const edge of edges) {
    ...
    const highlighted = Boolean((hoveredId && (edge.source === hoveredId || edge.target === hoveredId)) || (selectedId && (edge.source === selectedId || edge.target === selectedId)));
  ...
  for (const node of nodes) {
    ...
    if (highlighted || node.type === 'subject' || node.size >= 15) {
      context.font = node.type === 'subject' ? '650 14px Inter, system-ui, sans-serif' : '600 11px Inter, system-ui, sans-serif';
      context.fillStyle = 'rgba(255,255,255,0.92)';
      context.textAlign = 'center';
      const labelY = node.type === 'subject' || node.type === 'relation' ? node.y + node.size + 26 : node.y - node.size - 10;
...
function drawTechnicalBackdrop(context: CanvasRenderingContext2D, width: number, height: number, time: number) {
  ...
  for (let row = 0; row < 12; row += 1) {
    ...
    for (let x = -40; x <= width + 40; x += 28) {
      const y = baseY
        + Math.sin(x * 0.008 + row * 0.72 + drift) * 22
        + Math.sin(x * 0.017 - row * 0.45 + drift * 0.7) * 8;
...
function drawNode(context: CanvasRenderingContext2D, node: SimNode, imageCache: Map<string, CachedImage>, highlighted: boolean) {
  const color = nodeColors[node.type];
...
function tick(nodes: SimNode[], edges: GraphEdge[], width: number, height: number) {
  ...
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    for (let otherIndex = index + 1; otherIndex < nodes.length; otherIndex += stride) {
      ...
      const collisionBoost = distance < collisionDistance ? 1 + (collisionDistance - distance) / collisionDistance * 5 : 1;
...
function GraphCanvas({ graph }: { graph: GraphData }) {
  ...
  function findNodeAt(clientX: number, clientY: number) {
    ...
    return nodesRef.current.find((node) => Math.hypot(node.x - point.x, node.y - point.y) <= node.size + 12 / viewRef.current.scale) ?? null;
  }
  ...
  function fitView() {
    ...
    const scale = Math.min(1.45, Math.max(0.42, Math.min((rect.width - 80) / graphWidth, (rect.height - 80) / graphHeight)));
    commitView({
      scale,
  ...
  return (
    <div className="graph-stage">
      ...
      <div className="graph-toolbar">
        <Button aria-label={t('graph.zoomOut')} size="icon" type="button" variant="secondary" onClick={() => zoomAt(window.innerWidth / 2, window.innerHeight / 2, view.scale / 1.18)}>
          <Minus className="size-4" />
        </Button>
        <span>{Math.round(view.scale * 100)}%</span>
        <Button aria-label={t('graph.zoomIn')} size="icon" type="button" variant="secondary" onClick={() => zoomAt(window.innerWidth / 2, window.innerHeight / 2, view.scale * 1.18)}>
          <Plus className="size-4" />
        </Button>
        <Button aria-label={t('graph.fit')} size="icon" type="button" variant="secondary" onClick={fitView}>
          <Maximize2 className="size-4" />
        </Button>
        <Button aria-label={t('graph.reset')} size="icon" type="button" variant="secondary" onClick={() => commitView({ scale: 1, offsetX: 0, offsetY: 0 })}>
      ...
      {activeNode ? (
        <div className="graph-inspector">
          <div className="graph-inspector-media" style={{ color: nodeColors[activeNode.type] }}>
            {activeNode.image ? <img alt="" src={activeNode.image} /> : <div className="graph-inspector-orb" style={{ background: nodeColors[activeNode.type] }} />}
          </div>
          <div className="graph-inspector-main">
            ...
            {activeNode.details?.length ? (
              <div className="graph-inspector-details">
                {activeNode.details.slice(0, 6).map((detail) => <small key={detail}>{detail}</small>)}
              </div>
            ) : null}
          </div>
          {activeNode.href ? (
...
export function SubjectGraphPage() {
  ...
  }, [graphPayloadQuery.data?.characters, graphPayloadQuery.data?.episodes, graphPayloadQuery.data?.relations, graphPayloadQuery.data?.staff, sections, subject, t]);
  ...
  const layerCounts: Record<(typeof graphSectionKeys)[number], number> = {
    ...
    meta: (subject?.tags?.length ?? 0) + [subject?.platform, subject?.date, subject?.subject_type].filter(Boolean).length,
  };
  ...
  return (
    <section className="graph-fullscreen">
        ...
        <div className="graph-control-block">
          ...
          <div className="graph-mode-switch">
            {(['balanced', 'complete'] as const).map((value) => (
              <button className={mode === value ? 'is-active' : ''} key={value} type="button" onClick={() => setMode(value)}>
      ...
      {isLoading ? <div className="graph-state"><LoadingState title={t('graph.loading')} /></div> : null}
      {isError ? <div className="graph-state"><ErrorState title={t('subject.errorTitle')} description={t('subject.errorBody')} /></div> : null}
      {!isLoading && !isError && graph && graph.nodes.length <= 1 ? <div className="graph-state"><EmptyState title={t('graph.emptyTitle')} description={t('graph.emptyBody')} /></div> : null}
