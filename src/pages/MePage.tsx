...
import { activitiesApi } from '@/features/social/api';
import { socialQueries, socialQueryKeys } from '@/features/social/social-queries';
...
const labels: Record<Locale, Record<string, string>> = {
  'zh-CN': {
    ...
    noActivityBody: '标记作品或写文章后，这里会形成时间线。',
    loadingActivity: '正在加载动态',
    loadMore: '加载更多',
    loadingMore: '正在加载',
    activityEnd: '已经看到最新动态。',
  },
  'en-US': {
    profile: 'Profile',
    loading: 'Loading profile.',
    checking: 'Checking session',
    description: 'Your marks, reviews, and public activity live here.',
    ...
    loadMore: 'Load more',
    loadingMore: 'Loading',
    activityEnd: 'You are caught up.',
  },
  'ja-JP': {
    profile: 'プロフィール',
    loading: 'プロフィールを読み込んでいます。',
    checking: 'セッションを確認中',
    description: '公開アクション、記録、レビューがここに表示されます。',
    ...
    more: '多',
    activity: '活動記録',
    noActivityTitle: '記録はまだありません。',
    noActivityBody: '作品を記録したりレビューを書くと、ここにタイムラインが作られます。',
    loadingActivity: '記録を読み込み中',
    loadMore: 'さらに読み込む',
    loadingMore: '読み込み中',
    activityEnd: '最新の記録まで表示しました。',
  },
};
...
export function MePage() {
  ...
  const activityQuery = useInfiniteQuery({
    queryKey: [...socialQueryKeys.myActivities({ page_size: activityPageSize, ordering: '-created_at' }), 'stream'] as const,
    queryFn: ({ pageParam }) => activitiesApi.listMine({ page: pageParam, page_size: activityPageSize, ordering: '-created_at' }),
    initialPageParam: 1,
  ...
  const publicProfileQuery = useQuery({
    ...socialQueries.publicProfile(publicUserId),
  ...
  const activityItems = useMemo(() => activityQuery.data?.pages.flatMap((page) => page.results) ?? [], [activityQuery.data?.pages]);
  ...
  if (auth.status === 'checking') {
    return (
      <Page title={copy.profile} eyebrow={t('nav.groupOverview')}>
  ...
  return (
    <Page title={profile?.nickname ?? copy.profile} eyebrow={t('nav.groupOverview')} description={profile?.bio || undefined}>
          ...
          <section className="lg:sticky lg:top-6">
            <div className="flex items-center gap-4 lg:block">
              ...
              <div className="min-w-0 lg:mt-5">
                <h2 className="truncate text-2xl font-semibold tracking-tight text-[var(--color-text)]">{profile?.nickname}</h2>
                <p className="mt-1 truncate text-sm text-neutral-500 dark:text-neutral-400">{profile?.email}</p>
                {profile?.bio ? <p className="mt-3 line-clamp-4 text-sm leading-6 text-neutral-600 dark:text-neutral-300 lg:line-clamp-none">{profile.bio}</p> : null}
              </div>
            </div>
            ...
            {profile?.user_id ? (
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                >
                  <Users className="size-4" />
                  <strong className="font-semibold text-[var(--color-text)]">{publicStats?.following_count ?? 0}</strong>
            ...
            <nav className="mt-5 grid gap-1 border-t border-[var(--color-border)] pt-4 text-sm">
              <Link className="flex items-center justify-between gap-3 rounded-lg px-2 py-2 text-[var(--color-text-muted)] transition hover:bg-[var(--color-surface-muted)] hover:text-[var(--color-text)]" to={routes.library}>
              ...
              <Link className="flex items-center justify-between gap-3 rounded-lg px-2 py-2 text-[var(--color-text-muted)] transition hover:bg-[var(--color-surface-muted)] hover:text-[var(--color-text)]" to={routes.reviews}>
                <span className="inline-flex items-center gap-2">
              ...
              <Link className="flex items-center justify-between gap-3 rounded-lg px-2 py-2 text-[var(--color-text-muted)] transition hover:bg-[var(--color-surface-muted)] hover:text-[var(--color-text)]" to={routes.collections}>
                <span className="inline-flex items-center gap-2">
            ...
            <div className="grid min-w-0 gap-4">
              ...
              <div className="grid gap-0">
                {activityItems.map((activity) => (
                  <article className="relative grid grid-cols-[24px_minmax(0,1fr)] gap-3 border-l border-[var(--color-border)] pb-5 pl-4 last:pb-0" key={activity.id}>
                    ...
                    <div className="min-w-0">
                      ...
                      {activity.subject ? (
                        <Link className="mt-3 grid min-w-0 grid-cols-[48px_minmax(0,1fr)] gap-3 rounded-lg border border-[var(--color-border)] p-2 transition hover:border-[var(--color-accent-border)] hover:bg-[var(--color-surface-muted)]" to={routes.subject(activity.subject.id)}>
                          <img
              ...
              {!activityQuery.isLoading && activityItems.length > 0 ? (
                <div className="community-load-more">
                  {activityQuery.hasNextPage ? (
                    <Button disabled={activityQuery.isFetchingNextPage} type="button" variant="secondary" onClick={() => void activityQuery.fetchNextPage()}>
                      {activityQuery.isFetchingNextPage ? copy.loadingMore : copy.loadMore}
                    </Button>
                  ) : (
