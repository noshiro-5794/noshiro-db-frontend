...
export function PublicUserContentPage({ mode }: PublicUserContentPageProps) {
  ...
  const hasFilters = Boolean(keyword || (mode === 'subjects' && (status || subjectType)) || ordering !== defaultOrderingOf(mode));
        ...
        <main className="grid min-w-0 content-start gap-5">
          ...
          <div className="content-summary-bar">
            <div className="content-summary-count">
              ...
              <span className="inline-flex items-center gap-2">{modeIcon(mode)} {modeCountLabel(mode, t)}</span>
          ...
          {mode === 'reviews' ? (
            <div className="review-list">
              {(reviewsQuery.data?.results ?? []).map((review) => <PublicReviewItem key={review.id} review={review} />)}
            </div>
          ) : null}

          {mode === 'subjects' ? (
            <div className="content-list-panel">
              {(subjectsQuery.data?.results ?? []).map((item) => <PublicSubjectListItem key={item.id} item={item} />)}
            </div>
          ) : null}

          {mode === 'collections' ? (
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
              {(collectionsQuery.data?.results ?? []).map((collection) => <PublicCollectionPackCard key={collection.id} collection={collection} userId={userId} />)}
            </div>
          ) : null}
