          ...
          <section className="content-reader">
            <header className="content-reader-header">
              <div className="content-reader-author">
                <img
                  alt=""
                  src={profileQuery.data?.avatar || '/assets/placeholders/avatar.png'}
                />
              ...
              <div className="content-reader-badges">
                ...
                <Badge variant="secondary">{collectionQuery.data.item_count ?? itemsQuery.data?.count ?? 0} {t('common.items')}</Badge>
                <PublicRatingStars emptyLabel={t('common.unrated')} ratingLabel={t('library.simpleRatingLabel')} value={collectionQuery.data.simple_rating} />
              </div>
            </header>
            {collectionQuery.data.note ? (
              <p className="content-reader-note">{collectionQuery.data.note}</p>
            ) : null}
            <div className="content-reader-section-header">
              <h2>{t('collections.itemsTitle')}</h2>
              <span>{itemsQuery.data?.count ?? collectionQuery.data.item_count ?? 0} {t('common.items')}</span>
            </div>
            ...
            <div className="collection-rail content-reader-rail" aria-label={t('collections.itemsTitle')}>
              {items.map((item) => <PublicCollectionRailItem item={item} key={item.id} />)}
            </div>
