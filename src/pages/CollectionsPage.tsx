...
type CollectionOrdering = 'id' | '-id' | 'name' | '-name' | 'simple_rating' | '-simple_rating' | 'item_count' | '-item_count';
...
function userSubjectSubtitle(item: UserSubject, fallback: string) {
  const parts = [item.subject.subject_type, item.subject.date, item.subject.platform, item.status.replaceAll('_', ' ')].filter(Boolean);
  return parts.join(' / ') || fallback;
}
      ...
      {[1, 2, 3, 4, 5].map((rating) => {
        ...
        const star = (
          <Star
            className={cn(
              'size-4',
              filled ? 'fill-[var(--color-accent)] text-[var(--color-accent)]' : 'text-neutral-300 dark:text-neutral-700',
            )}
          />
        );
          ...
          <DialogContent>
            ...
            <form className="grid gap-4" onSubmit={handleCreateCollection}>
              <label className="grid gap-2 text-sm font-medium text-neutral-700 dark:text-neutral-300">
                {t('collections.name')}
                <Input value={newName} placeholder={t('collections.namePlaceholder')} onChange={(event) => setNewName(event.target.value)} />
              </label>
              <label className="grid gap-2 text-sm font-medium text-neutral-700 dark:text-neutral-300">
                {t('collections.note')}
                <textarea
              ...
              <div className="flex items-center justify-between gap-4 rounded-xl border border-neutral-200 px-4 py-3 dark:border-neutral-800">
                <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">{t('collections.rating')}</span>
    ...
    >
      <div className="grid min-w-0 gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
        <aside className="grid content-start gap-4">
          ...
          {collectionsQuery.isError ? <ErrorState title={t('reviews.errorTitle')} description={t('search.errorBody')} /> : null}
          ...
          <div className="grid gap-2">
            {collections.map((collection) => {
              ...
              return (
                <button
                  className={cn(
                    'collection-pack-card group text-left transition',
                    isSelected
                      ? 'is-selected'
                      : '',
                  )}
                ...
                >
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    ...
                    {collection.is_public ? <Eye className="size-4 opacity-60" /> : <EyeOff className="size-4 opacity-60" />}
                  </div>
                </button>
              );
            })}
            ...
            <div className="grid min-w-0 gap-5">
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Badge>{itemCount} {t('common.items')}</Badge>
                    </div>
                    <h2 className="mt-3 text-2xl font-semibold tracking-tight text-neutral-950 dark:text-white">{collectionTitle(selectedCollection, t('collections.chooseTitle'))}</h2>
                    {selectedCollection.note ? (
                      <p className="mt-2 max-w-3xl text-sm leading-6 text-neutral-500 dark:text-neutral-400">{selectedCollection.note}</p>
                    ) : (
                    ...
                    <div className="mt-4 flex items-center gap-3">
                      ...
                      <span className="text-sm text-neutral-500 dark:text-neutral-400">{starLabel(selectedCollection.simple_rating, t('common.unrated'))}</span>
                    </div>
                  </div>
                    ...
                    <Dialog open={addOpen} onOpenChange={setAddOpen}>
                      ...
                      <DialogContent className="max-h-[calc(100dvh-2rem)] max-w-3xl overflow-hidden p-0">
                        ...
                        <div className="grid max-h-[calc(100dvh-9rem)] gap-4 overflow-y-auto p-5">
                          ...
                          {librarySearchQuery.isError ? <ErrorState title={t('library.errorTitle')} description={t('search.errorBody')} /> : null}
                          <div className="collection-library-results">
                            {(librarySearchQuery.data?.results ?? []).map((item) => (
                              <article className="collection-library-item" key={item.id}>
                                <img alt="" src={userSubjectImage(item) || '/assets/placeholders/subject-cover.png'} loading="lazy" />
                                <div className="min-w-0">
                    ...
                    <Dialog open={editOpen} onOpenChange={setEditOpen}>
                      ...
                      <DialogContent>
                        ...
                        <form className="grid gap-4" onSubmit={handleUpdateCollection}>
                          ...
                          <div className="flex items-center justify-between gap-4 rounded-xl border border-neutral-200 px-4 py-3 dark:border-neutral-800">
                            <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">{t('collections.rating')}</span>
                            <RatingStars interactive label={t('common.unrated')} value={editRating} onChange={setEditRating} />
                          </div>
                          <button
                            className="flex items-center justify-between gap-4 rounded-xl border border-neutral-200 px-4 py-3 text-left text-sm font-medium text-neutral-700 transition hover:bg-neutral-50 dark:border-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-900"
                            type="button"
                          ...
                          >
                            <span>{editIsPublic ? t('collections.publicCollection') : t('collections.privateCollection')}</span>
              ...
              <div className="min-w-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-950">
                <div className="flex flex-col gap-3 border-b border-neutral-200 p-4 dark:border-neutral-800 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {isOrderDirty ? (
                      <Button disabled={!canSaveOrder || updateItemsMutation.isPending} size="sm" type="button" onClick={saveOrder}>
                ...
                {itemsQuery.isError ? <ErrorState title={t('collections.itemsErrorTitle')} description={t('search.errorBody')} /> : null}
                  ...
                  {items.map((item, index) => (
                    ...
                    >
                      ...
                      <Link className="collection-rail-poster" to={routes.subject(item.subject.id)}>
                        {subjectImage(item) ? (
                          <img src={subjectImage(item) ?? ''} alt="" loading="lazy" />
                        ) : (
                          <span />
                        )}
                      </Link>
