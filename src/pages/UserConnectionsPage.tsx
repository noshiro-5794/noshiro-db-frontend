...
import type { FollowRelation } from '@/lib/api/types';
    ...
    <article className="community-list-item">
      <div className="grid min-w-0 grid-cols-[44px_minmax(0,1fr)_auto] items-center gap-3">
        ...
        <div className="min-w-0">
          <Link className="line-clamp-1 text-sm font-semibold text-neutral-950 transition hover:text-[var(--color-accent-strong)] dark:text-white" to={routes.userProfile(user.id)}>
...
export function UserConnectionsPage({ mode }: UserConnectionsPageProps) {
  ...
  const myFollowersQuery = useQuery({
    ...socialQueries.myFollowers({ page: currentPage, page_size: pageSize }),
    enabled: isValidUserId && isSelf && auth.isAuthenticated && mode === 'followers',
  });
  const myFollowingQuery = useQuery({
    ...socialQueries.myFollowing({ page: currentPage, page_size: pageSize }),
    enabled: isValidUserId && isSelf && auth.isAuthenticated && mode === 'following',
  });
  const followersQuery = useQuery({
    ...socialQueries.publicFollowers(userId, { page: currentPage, page_size: pageSize }),
    enabled: isValidUserId && (!isSelf || !auth.isAuthenticated) && mode === 'followers',
  });
  const followingQuery = useQuery({
    ...socialQueries.publicFollowing(userId, { page: currentPage, page_size: pageSize }),
    enabled: isValidUserId && (!isSelf || !auth.isAuthenticated) && mode === 'following',
  });
  const listQuery = isSelf && auth.isAuthenticated
    ? mode === 'followers' ? myFollowersQuery : myFollowingQuery
  ...
  return (
    ...
    >
      ...
      {profileQuery.isError || listQuery.isError ? <ErrorState title={t('profile.connectionsErrorTitle')} description={t('profile.connectionsErrorBody')} /> : null}
      {!listQuery.isLoading && !listQuery.isError && relations.length === 0 ? (
        <EmptyState
          title={emptyTitle}
          description={emptyBody}
          action={<Users className="size-4 text-neutral-400" />}
        />
      ) : null}
      <div className="community-list">
        {relations.map((relation) => <ConnectionItem key={`${relation.user.id}-${relation.followed_at}`} relation={relation} />)}
      </div>
