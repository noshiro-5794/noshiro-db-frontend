...
async function createSquareAvatarFile(file: File, sourceUrl: string, zoom: number, offset: AvatarOffset) {
  ...
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((result) => (result ? resolve(result) : reject(new Error('Unable to crop image.'))), 'image/webp', 0.92);
  });
...
export function SettingsPage() {
  ...
  const avatarCropGeometry = avatarDraft ? getAvatarCropGeometry(avatarDraft.width, avatarDraft.height, avatarZoom, avatarOffset) : null;
        ...
        <aside className="grid content-start gap-6">
          <section className="lg:sticky lg:top-6">
            <div className="flex items-center gap-4 lg:block">
              <img className="size-24 rounded-full bg-neutral-100 object-cover ring-1 ring-neutral-200 dark:bg-neutral-900 dark:ring-neutral-800 lg:size-28" src={profile.avatar || avatarPlaceholder} alt="" />
          ...
          {avatarDraft ? (
            <div className="grid gap-5">
              ...
              <div className="flex flex-wrap justify-end gap-2">
                ...
                <Button disabled={isUploading} type="button" variant="secondary" onClick={() => void uploadAvatarFile(avatarDraft.file)}>
                  <Upload className="size-4" />
