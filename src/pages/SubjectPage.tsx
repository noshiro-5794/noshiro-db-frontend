...
const importantStaffRoles = ['監督', '导演', 'director', '原作', '脚本', '系列构成', 'シリーズ構成', 'キャラクターデザイン', '角色设计', '音楽', '音乐', '动画制作'];
...
function episodeLabel(episode: SubjectEpisode) {
  ...
  return [episode.type, episode.sort ?? episode.ep_num].filter((item) => item !== null && item !== undefined && item !== '').join(' ');
}
    ...
    return value
      .map((item) => {
        ...
        if (item && typeof item === 'object') {
          ...
          return [record.v, record.name, record.title].find((entry) => typeof entry === 'string') as string | undefined;
        }
        return '';
      })
      .filter(Boolean)
...
function relationMeta(relation: SubjectRelation, fallback: string) {
  ...
  const contentMeta = [subject.content?.episodes ? `${subject.content.episodes} EP` : '', subject.content?.volumes ? `${subject.content.volumes} Vol` : ''].filter(Boolean);
  return [...displayMeta, subject.display_subtitle, subject.date, subject.platform, ...contentMeta].filter(Boolean).join(' · ') || fallback;
}
...
function episodeMeta(episode: SubjectEpisode, fallback: string) {
  return [episode.date, episode.duration, episode.sort !== null && episode.sort !== undefined ? `sort ${episode.sort}` : ''].filter(Boolean).join(' · ') || fallback;
}

function StarRatingControl({ clearLabel, disabled, value, onChange }: { clearLabel: string; disabled?: boolean; value: string; onChange: (value: string) => void }) {
...
function StaffDetail({ emptyLabel, labels, staff, role }: { emptyLabel: string; labels: { birth: string; career: string; gender: string; type: string }; staff: SubjectStaff; role?: string }) {
  return (
    <DetailShell
      ...
      rows={[
        ...
        ...getInfoboxRows(staff.infobox).slice(0, 6).map((item) => [item.key, item.value] as const),
      ]}
    />
  );
}
            ...
            <div className="mt-4 grid gap-3">
              {staffGroups.map(([role, members]) => (
                <div className="grid gap-2" key={role}>
                  <div className="text-xs font-semibold uppercase text-neutral-400">{role === 'Staff' ? t('subject.staff') : role}</div>
                  <div className="grid gap-2">
                    {members.map((member) => (
                      <Popover key={`${role}-${member.id}`}>
                        ...
                        <PopoverContent align="start" className="max-h-[min(640px,calc(100dvh-2rem))] w-[min(420px,calc(100vw-2rem))] overflow-y-auto p-4" side="right">
                          <StaffDetail emptyLabel={emptyText} labels={subjectDetailLabels} role={role === 'Staff' ? t('subject.staff') : role} staff={member} />
                        </PopoverContent>
                      </Popover>
                    ))}
                  </div>
              ...
              {staffQuery.isFetching ? <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('subject.loadingStaff')}</p> : null}
        ...
        <main className="grid gap-4">
          <div className="grid gap-4 lg:grid-cols-2">
            <Card className={anchorScrollClass} id="mark">
              ...
              <CardContent className="grid gap-4">
                ...
                {isAuthenticated ? (
                  <Button className="mark-primary-action" type="button" variant="secondary" onClick={() => setIsMarkDialogOpen(true)}>
                  ...
                  >
                    <span className="episode-tile-topline">
                      ...
                      >
                        ...
                        {isEpisodePending ? t('common.saving') : checked ? t('subject.watched') : t('subject.markWatched')}
                      </button>
                    </span>
                    <span className="episode-tile-title">
                    ...
                    {otherEpisodeRows.map((episode) => (
                      ...
                      >
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="other-chapter-meta">{episodeLabel(episode)} · {episodeMeta(episode, emptyText)}</span>
                        </span>
                      </button>
                    ))}
                  </div>
                        ...
                        {(character.actors ?? []).slice(0, 2).map((actor) => (
                          <div className="grid grid-cols-[24px_minmax(0,1fr)] items-center gap-2" key={actor.id}>
                            <img
                              ...
                              alt=""
                            />
                            <span className="truncate text-xs text-neutral-600 dark:text-neutral-300">{actor.name}</span>
                          </div>
                        ))}
          ...
          <Card className={anchorScrollClass} id="relations">
            <CardContent className="grid gap-5 pt-6">
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold tracking-tight text-neutral-950 dark:text-white">{t('subject.relations')}</h2>
              ...
              {relationsQuery.isError ? (
                <div className="grid gap-3">
                  ...
                  <Button className="w-fit" type="button" variant="secondary" onClick={() => void relationsQuery.refetch()}>
                    {t('common.retry')}
                  </Button>
                </div>
              ) : null}
          ...
          <Card className={anchorScrollClass} id="public-reviews">
            <CardContent className="grid gap-4 pt-6">
              <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold tracking-tight text-neutral-950 dark:text-white">{t('subject.publicReviews')}</h2>
                <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            ...
            <div className="mt-4 divide-y divide-neutral-200 dark:divide-neutral-800">
                {(publicReviewsQuery.data?.results ?? []).map((review) => {
                  const isOwnReview = Boolean(review.user?.id && auth.profile?.user_id && String(review.user.id) === String(auth.profile.user_id));
                  return (
                  ...
                  {ratingDetailRows.map((detail, index) => (
                    <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_40px]" key={index}>
                      <Input
                        ...
                        value={detail.key}
                        onChange={(event) =>
                          setRatingDetailRows((rows) => rows.map((row, rowIndex) => (rowIndex === index ? { ...row, key: event.target.value } : row)))
                        }
                      />
                      <Input
                        placeholder="value"
                        value={detail.value}
                        onChange={(event) =>
                          setRatingDetailRows((rows) => rows.map((row, rowIndex) => (rowIndex === index ? { ...row, value: event.target.value } : row)))
                        }
                      />
                      <Button
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  ...
                  ].filter(Boolean).join(' · ') || t('subject.episodeDetail')}
                </DialogDescription>
              </DialogHeader>
              <DetailShell
          ...
          {selectedCharacter ? (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {[selectedCharacter.role, selectedCharacter.type].filter(Boolean).join(' · ') || t('subject.characterDetail')}
                </DialogDescription>
              </DialogHeader>
              <DetailShell
                ...
                rows={[
                  ...
                  ...getInfoboxRows(selectedCharacter.infobox).slice(0, 8).map((item) => [item.key, item.value] as const),
                ]}
              >
          ...
          {selectedStaff ? (
            <>
              <DialogHeader>
                ...
                <DialogDescription>{selectedStaff.role || selectedStaff.type || t('subject.staffDetail')}</DialogDescription>
              </DialogHeader>
          ...
          {selectedRelation ? (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {[selectedRelation.subject.subject_type, selectedRelation.relation || t('subject.related')].filter(Boolean).join(' · ')}
                </DialogDescription>
              </DialogHeader>
              <DetailShell
                ...
                subtitle={[selectedRelation.relation || t('subject.related'), selectedRelation.subject.subject_type].filter(Boolean).join(' · ')}
