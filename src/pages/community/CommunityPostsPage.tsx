...
import type { Activity, ActivityType, SubjectSummary } from '@/shared/api';
  ...
  return (
    <article className="activity-timeline-item">
      <div className="activity-timeline-avatar">
        {author?.id ? (
          <Link to={routes.userProfile(author.id)}>
            <img className="activity-avatar" src={author.avatar || '/assets/placeholders/avatar.png'} alt="" />
          </Link>
        ) : (
      ...
      <div className="activity-timeline-body">
        ...
        {body ? (
          <p
            className={`activity-body-copy ${activity.post?.is_spoiler || activity.review?.is_spoiler ? 'is-spoiler' : ''}`}
          >
            {body}
          </p>
        ) : null}
      ...
      actions={
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          <DialogTrigger asChild>
            <Button className="activity-new-post-button" type="button">
              <MessageSquare className="size-4" /> {t('community.newPost')}
            </Button>
          </DialogTrigger>
