...
import { syncMutations, syncQueries, syncQueryKeys } from '@/features/admin-sync';
...
function isIncrementalResult(
  value: unknown,
): value is { results?: IncrementalSyncResult[] } & Partial<IncrementalSyncResult> {
      ...
      {isIncrementalResult(data) && getIncrementalResults(data).length > 0 ? (
        <div className="grid gap-2">
          {getIncrementalResults(data).map((item) => (
            <div
              className="grid gap-2 rounded-lg bg-neutral-50 p-3 text-sm dark:bg-neutral-900/70 sm:grid-cols-[minmax(0,1.4fr)_repeat(4,minmax(0,0.7fr))]"
              key={`${item.task_name}-${item.shard}`}
            >
              ...
              {item.frontier_reached ? (
                <span className="text-xs font-semibold text-[var(--color-accent-strong)] sm:col-span-5">
                  Frontier reached
                </span>
              ) : null}
            </div>
...
function StatusList({ tasks }: { tasks: SyncTaskStatus[] }) {
  return (
    <div className="grid gap-2">
      {tasks.map((task) => (
        <div
          className="grid gap-2 rounded-lg border border-neutral-200 p-3 text-sm dark:border-neutral-800 md:grid-cols-[minmax(0,1fr)_120px_120px_120px]"
        ...
        {jobs.length === 0 ? (
          ...
          <div className="grid gap-2">
            {jobs.map((job) => {
              ...
              return (
                <div
                  className="grid gap-3 rounded-lg border border-neutral-200 p-3 dark:border-neutral-800"
                  key={job.id}
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                  ...
                  <div className="grid gap-2 text-xs text-neutral-500 dark:text-neutral-400 sm:grid-cols-4">
                    <span>
                      {job.synced_count} {t('admin.synced')}
                    </span>
                    <span>
                      {job.skipped_count} {t('admin.skipped')}
                    </span>
                    <span>
                      {job.failed_count} {t('admin.failed')}
                    </span>
                    <span>
                      {job.started_at
                        ? `${t('admin.started')} ${new Date(job.started_at).toLocaleTimeString()}`
                        : t('admin.waiting')}
                    </span>
                  </div>
                  {job.error ? (
                    <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600 dark:bg-red-950/25 dark:text-red-300">
                      {job.error}
                    </p>
                  ) : null}
                </div>
          ...
          <Card>
            ...
            <CardContent className="grid gap-5">
              ...
              >
                <label className="grid gap-2">
                  <span className="text-sm font-medium text-neutral-700 dark:text-neutral-200">
                    {t('admin.bangumiSubject')}
                  </span>
                  <Input
                    inputMode="numeric"
                    placeholder={t('admin.bangumiPlaceholder')}
                    value={bangumiId}
                    onChange={(event) => setBangumiId(event.target.value)}
                  />
                </label>
              ...
              <div className="grid gap-3 border-t border-neutral-200 pt-5 dark:border-neutral-800 sm:grid-cols-2">
                ...
                <label className="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-300">
                  <input
                    checked={syncSubjectDetails}
                    className="size-4 accent-[var(--color-accent)]"
                    type="checkbox"
                    onChange={(event) => setSyncSubjectDetails(event.target.checked)}
                  />
                  {t('admin.syncDetails')}
                </label>
              </div>
