            ...
            <div className="border-y border-[color-mix(in_srgb,var(--color-border)_64%,transparent)]">
              {page.sections.map((section) => (
                <section
                  ...
                  key={section.id}
                >
                  <h2 className="text-xl font-semibold tracking-tight text-[var(--color-text)] sm:text-2xl">{section.title}</h2>
        ...
        <aside className="hidden xl:block">
          <div className="sticky top-8 grid gap-3 rounded-3xl border border-[color-mix(in_srgb,var(--color-border)_62%,transparent)] bg-[color-mix(in_srgb,var(--color-surface)_58%,transparent)] p-4 text-sm shadow-sm">
            ...
            <nav className="grid gap-2">
              {page.sections.map((section) => (
                <a className="text-[var(--color-text-muted)] transition hover:text-[var(--color-text)]" href={`#${section.id}`} key={section.id}>
                  {section.title}
                </a>
              ))}
            </nav>
