  ...
  return (
    <Link className="group grid min-w-0 gap-2" state={state} to={to}>
      ...
      <span className="min-w-0">
        ...
        <span className="mt-1 line-clamp-1 text-xs text-neutral-500 dark:text-neutral-400">{subtitle}</span>
      </span>
    </Link>
  );
}
...
export function SearchPage() {
  ...
  const subjectQueryParams = useMemo(
    ...
    [
      currentPage,
      episodeRangeParams.max,
      episodeRangeParams.min,
      keyword,
      ordering,
      platform,
      safety,
  ...
  const allFilters: SearchFilterKey[] = [
    'type',
    'sourceId',
    'year',
    'season',
  ...
  return (
    <Page title={t('search.title')} eyebrow={t('nav.groupDiscover')} hideHeader={role === 'guest'} seo={false}>
      <Seo
                ...
                {filter === 'year' ? (
                  <FilterCombobox
                    allowCustomValue={(value) =>
                      /^\d{4}$/u.test(value) && Number(value) >= 1900 && Number(value) <= 2100
                    }
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-7 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          {shouldUseSubjectSearch
            ...
            : visibleCalendarItems.map((item) => (
                <SearchPoster
                  badge={new Intl.NumberFormat(locale, {
                    notation: item.doing >= 10000 ? 'compact' : 'standard',
                  }).format(item.doing)}
                  key={item.subject_id}
