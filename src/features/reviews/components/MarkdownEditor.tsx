...
export function MarkdownEditor({ value, onChange }: MarkdownEditorProps) {
  ...
  const toolbarGroups = [
    ...
    [
      ...
      { label: t('reviewEditor.table'), icon: Table2, prefix: '\n| Key | Value |\n| --- | --- |\n|  |  |\n', suffix: '' },
    ],
  ];
  const stats = useMemo(
    () => ({
      ...
      <div className="review-editor">
        <header className="review-editor-toolbar">
          ...
          <div className="review-editor-actions">
            <div className="review-editor-status">
              ...
              <span>{stats.minutes} {t('common.min')}</span>
            </div>
            <div className="review-editor-mode">
              <Button aria-label={t('reviewEditor.write')} size="icon" type="button" variant={mode === 'write' ? 'secondary' : 'ghost'} onClick={() => setMode('write')}>
                <PanelLeftClose className="size-4" />
              </Button>
              <Button aria-label={t('reviewEditor.split')} size="icon" type="button" variant={mode === 'split' ? 'secondary' : 'ghost'} onClick={() => setMode('split')}>
                <Split className="size-4" />
              </Button>
              <Button aria-label={t('reviewEditor.preview')} size="icon" type="button" variant={mode === 'preview' ? 'secondary' : 'ghost'} onClick={() => setMode('preview')}>
        ...
        {outline.length ? (
          <nav aria-label={t('reviewEditor.outline')} className="review-editor-outlinebar">
            ...
            <div>
              {outline.map((item) => (
                <button data-depth={item.depth} key={`${item.line}-${item.title}`} type="button" onClick={() => focusLine(item.line)}>
                  {item.title}
                </button>
              ))}
            </div>
        ...
        <div className={`review-editor-grid is-${mode}`}>
          <div className="review-editor-pane review-editor-write">
            <div className="review-editor-pane-header">
              <span><PencilLine className="size-4" /> {t('common.markdown')}</span>
              <span>{stats.words} {t('common.words')}</span>
            </div>
            <textarea
          ...
          <div className="review-editor-pane review-editor-preview">
            <div className="review-editor-pane-header">
              <span><Eye className="size-4" /> {t('reviewEditor.preview')}</span>
              <span>{stats.minutes} {t('common.minRead')}</span>
            </div>
            <div className="review-editor-preview-body">
              {value.trim() ? <MarkdownRenderer content={value} /> : <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('reviewEditor.previewEmpty')}</p>}
            </div>
          </div>
        </div>
      </div>
