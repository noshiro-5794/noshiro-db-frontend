...
import type { I18nState } from '@/features/i18n/i18n-context-value';
...
export function notificationHref(notification: CommunityNotification) {
  ...
  if (target.type === 'activity') {
    ...
    if (target.collection?.id) return target.collection.owner?.id ? routes.userCollection(target.collection.owner.id, target.collection.id) : routes.collections;
    if (target.comment?.id) return withHash(target.user?.id ? routes.userProfile(target.user.id) : routes.home, `comment-${target.comment.id}`);
...
export function notificationMessage(t: Translate, notification: CommunityNotification) {
  ...
  if (notification.notification_type === 'commented') return `${t('community.notificationMessage.commented')} ${target}`;
  if (notification.notification_type === 'reacted') return `${t('community.notificationMessage.reacted')} ${target}`;
  if (notification.notification_type === 'mentioned') return `${t('community.notificationMessage.mentioned')} ${target}`;
