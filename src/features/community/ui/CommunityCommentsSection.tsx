...
import { Link, useSearchParams } from '@/shared/routing/navigation';
...
import type { CommunityCommentSummary, CommunityReportReason, CommunityTargetType } from '@/shared/api';
  ...
  return (
    <article className="timeline-comment-item scroll-mt-24" id={`comment-${comment.id}`}>
      ...
      <div className="min-w-0">
        ...
        <p
          className={`timeline-comment-copy ${isDeleted ? 'is-deleted' : ''} ${comment.is_spoiler && !isDeleted ? 'is-spoiler' : ''}`}
        >
          ...
          {replyTarget ? (
            <div className="timeline-reply-pill">
              <span>
                ...
                <strong className="text-[var(--color-text)]">{replyTargetName(replyTarget)}</strong>
              </span>
              <button
        ...
        <DialogContent>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              type="button"
              onClick={() => deleteTarget && deleteCommentMutation.mutate(deleteTarget.id)}
            >
        ...
        <DialogContent className="max-w-2xl">
          ...
          <form className="grid gap-4" onSubmit={submitEdit}>
            <textarea
              ...
              onChange={(event) =>
                setEditTarget((current) => (current ? { ...current, content: event.target.value } : current))
              }
            />
            <label className="inline-flex items-center gap-2 text-sm text-neutral-500 dark:text-neutral-400">
              <input
                ...
                type="checkbox"
                onChange={(event) =>
                  setEditTarget((current) => (current ? { ...current, is_spoiler: event.target.checked } : current))
                }
              />
              {t('community.markSpoiler')}
            </label>
