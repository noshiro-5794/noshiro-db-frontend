...
export function activityTargetHref(activity: Activity, fallback: string = routes.communityPosts, ownerId?: number | string) {
  ...
  if ((activity.activity_type === 'collection_created' || activity.activity_type === 'collection_item_added') && activity.collection?.id) {
    return collectionHref(activity, fallback, ownerId);
  }
  if ((activity.activity_type === 'user_subject_created' || activity.activity_type === 'user_subject_updated') && activity.subject?.id) {
    return routes.subject(String(activity.subject.id));
  }
  if (activity.activity_type === 'user_followed' && activity.target_user?.id) return routes.userProfile(activity.target_user.id);
