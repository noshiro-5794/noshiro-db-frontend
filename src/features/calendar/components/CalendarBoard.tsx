...
import { calendarImageOf } from '@/features/search/calendar-search';
...
function HeroCalendarItem({ item, locale, state, titleFallback }: { item: CalendarSubjectItem; locale: Locale; state: RouteBackState; titleFallback: string }) {
  return (
    <Link
...
function CompactCalendarItem({ item, locale, state, titleFallback }: { item: CalendarSubjectItem; locale: Locale; state: RouteBackState; titleFallback: string }) {
  return (
    <Link
    ...
    >
      ...
      <span className="grid min-w-0 grid-rows-[auto_1fr_auto] gap-1 py-0.5">
        <span className="line-clamp-2 text-sm font-semibold leading-5 text-[var(--color-text)]">{titleOf(item, titleFallback)}</span>
        ...
        <span className="flex min-w-0 items-center gap-2 text-xs font-medium text-[var(--color-text-muted)]">
          <span className="rounded-full bg-[var(--color-surface-muted)] px-2 py-0.5">{formatHeat(item.doing, locale)}</span>
          ...
          return (
            ...
            >
              <header className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <h3 className="text-sm font-semibold text-[var(--color-text)]">{weekdayLabels[locale][weekday]}</h3>
              ...
              {featuredItem ? (
                <HeroCalendarItem item={featuredItem} locale={locale} state={subjectLinkState} titleFallback={t('common.untitledSubject')} />
              ) : (
              ...
              <div className="mt-4 grid gap-2">
                {restItems.slice(0, 4).map((item) => (
                  <CompactCalendarItem item={item} key={item.subject_id} locale={locale} state={subjectLinkState} titleFallback={t('common.untitledSubject')} />
                ))}
              </div>
            </section>
          );
