  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button aria-label={t('nav.notifications')} className="workspace-tool-button relative" type="button">
        ...
        <div className="max-h-96 overflow-y-auto px-2 pb-2">
          ...
          {notificationPreview.map((notification) => {
            ...
            return (
              <Link
                className={[
                  ...
                  notification.is_read
                    ? ''
                    : 'bg-[color-mix(in_srgb,var(--color-accent-soft)_52%,var(--color-surface-muted))]',
                ].join(' ')}
                key={notification.id}
                to={href}
              ...
              >
                {!notification.is_read ? (
                  <span className="absolute left-1.5 top-3 size-1.5 rounded-full bg-[var(--color-accent)]" />
                ) : null}
